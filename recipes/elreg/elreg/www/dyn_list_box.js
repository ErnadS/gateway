...
form fields referencing the same list structure, and form resetting.
*/
//USAGE:
...
//dol.forValue("Value1").forValue("Value2").addOptions("1","2","3");
	...
	// If the options you want to add should have different values and display
...
var fieldListIndexes;

function getInstanceDL() {
return this;	
}
...
function DynamicOptionList() {
	...
	this.printPercentOptions =DOL_printPercentOptions;
	this.printPeriodOptions =DOL_printPeriodOptions;
...
function DOL_change(obj, usePreselected) {
	//alert("objectName" + obj.name + "select: " + obj.selectedIndex + " type: " + obj.type);
	/*if (typeof(fieldListIndexes) == "undefined"){
		alert("NULL" + "len: " + obj.options.length);
		return;
	}*/
	//alert("objectName" + obj.name + "len: " + fieldListIndexes.length);
/*	if (this.fieldListIndexes == null)
	alert("bb");
	else
	alert("IKKE");
	*/
	...
	if (child == null) {  } // No child, no need to continue
