...
char smtpConfig_save(SSMTP ssmtp) {
	...
	//fputs("\n", hFile);
	printf("aa2\n");
	if (ssmtp.sender[0] != 0 && strlen(ssmtp.sender) > 0) {
	...
	while (1) {
		...
		if (strcmp(paramName, "mailhub") == 0) {
			...
			if (sepPointer == NULL) {
				...
				//fprintf(stderr, "smtpAddress: %s port: %u \n", pssmtpStruct->smtpAddress, pssmtpStruct->port);
			}
