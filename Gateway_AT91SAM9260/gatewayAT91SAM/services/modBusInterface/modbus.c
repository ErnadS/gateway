...
#include "../../utility/MyGpio.h" // !!! TEST: 2014. Try to get RST on RS485 to work
...
#define USE_485  001000000000		/* ERNAD */

/* Internal use */
#define MSG_LENGTH_UNDEFINED -1
//int setDTR(int serial_fd, unsigned short level);

/* Exported version */
const unsigned int libmodbus_version_major = 1; //LIBMODBUS_VERSION_MAJOR;
const unsigned int libmodbus_version_minor = 1; //LIBMODBUS_VERSION_MINOR;
const unsigned int libmodbus_version_micro = 1; //LIBMODBUS_VERSION_MICRO;
...
static const int TAB_HEADER_LENGTH[2] = { HEADER_LENGTH_RTU, HEADER_LENGTH_TCP };

static const int TAB_CHECKSUM_LENGTH[2] = { CHECKSUM_LENGTH_RTU,
		CHECKSUM_LENGTH_TCP };

static const int TAB_MAX_ADU_LENGTH[2] = { MODBUS_MAX_ADU_LENGTH_RTU,
...
static int send_msg(modbus_t *ctx, uint8_t *req, int req_length) {
	...
	//usleep(9000);
	//setDTR(ctx->s, 0); // set DTR low (disable Send og enable read)
		...
		if (msg_length < msg_length_computed) {
			...
			case FUNCTION:
				...
				length_to_read = compute_header_length(
						msg[TAB_HEADER_LENGTH[ctx->type_com]], msg_type);
...
if	(ctx->debug) {
	...
	case FC_READ_COILS: {
		...
		printf("Address to read is: %u\n", address);
		printf("No.of bytes to read is: %u\n", nb);
		...
		if ((address + nb) > mb_mapping->nb_bits) {
			...
			// ES:
			//////////////////////////////////////////////////////////////
			// "ctx" is struct "_modbus" (type modbus, is debug ...).
			// "stf" is request info (slave address, commadn ...)
			// "rsp" is response which will be sent to slave
			//////////////////////////////////////////////////////////////
			rsp_length = build_response_basis(ctx, &sft, rsp); // add basic header
			rsp[rsp_length++] = (nb / 8) + ((nb % 8) ? 1 : 0); // add one byte msg length ("nb" is in bits?)
	...
	if (rc > 0) {
		...
		for (i = offset; i < offset_end; i++) {
			...
			for (bit = 0x01; (bit & 0xff) && (pos < nb);) {
				data_dest[pos++] = (temp & bit) ? 1 : 0; // true/false
				bit = bit << 1;
			}
...
int modbus_read_registers(modbus_t *ctx, int addr, int nb, uint16_t *data_dest) {
	...
	status = read_registers(ctx, FC_READ_HOLDING_REGISTERS, addr, nb,
			data_dest);
	return status;
}
...
int modbus_read_input_registers(modbus_t *ctx, int addr, int nb,
	...
	status = read_registers(ctx, FC_READ_INPUT_REGISTERS, addr, nb, data_dest);
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *data_src) {
	...
	req_length = build_request_basis(ctx, FC_WRITE_MULTIPLE_COILS, addr, nb,
			req);
...
int modbus_write_registers(modbus_t *ctx, int addr, int nb,
	...
	req_length = build_request_basis(ctx, FC_WRITE_MULTIPLE_REGISTERS, addr, nb,
			req);
	byte_count = nb * 2;
	req[req_length++] = byte_count;
...
int modbus_read_and_write_registers(modbus_t *ctx, int read_addr, int read_nb,
		uint16_t *dest, int write_addr, int write_nb, const uint16_t *data) {
	int rc;
	int req_length;
	int i;
	int byte_count;
...
int modbus_set_error_recovery(modbus_t *ctx, int enabled) {
	if (enabled == 1 || enabled == 0) { // true /false
...
/*
 ...
 printf("error setDTR() to level: %u\n", level);
 return 0;
 }
 if (level) {
 status |= TIOCM_DTR;
 ...
 printf("success setDTR to level: %u\n", level);
 return 1;
 }
 */
/////////////////////////////////////////////////////////////////////////////////
...
static int modbus_connect_tcp(modbus_t *ctx) {
	...
	/* SOL_TCP = IPPROTO_TCP */
	option = 1;
	rc = setsockopt(ctx->s, IPPROTO_TCP, TCP_NODELAY, (const void *) &option,
			sizeof(int));
	if (rc == -1) {
		close(ctx->s);
		return -1;
	...
	rc = setsockopt(ctx->s, IPPROTO_IP, IP_TOS, (const void *) &option,
			sizeof(int));
	if (rc == -1) {
		close(ctx->s);
		return -1;
...
void modbus_set_bits_from_bytes(uint8_t *dest, int address,
		unsigned int nb_bits, const uint8_t tab_byte[]) {
	int i;
	int shift = 0;
