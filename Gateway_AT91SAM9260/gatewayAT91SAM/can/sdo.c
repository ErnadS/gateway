...
** Returns:
**/
inline unsigned char _readNetworkDict (CO_Data* d, unsigned char nodeId, unsigned int index, unsigned char subIndex,
                                       unsigned char dataType, SDOCallback_t Callback);
...
{
#ifdef DEBUG_SDO
  printf("!!!!! SDO timeout. SDO response not received.\n");
  printf("server node : %d\n", d->transfers[id].nodeId);
  printf("index : %d\n", d->transfers[id].index);
  printf("subIndex : %d\n", d->transfers[id].subIndex);
#endif
...
//DELETED 24112014 T.S  else if(d->transfers[id].whoami == SDO_SERVER)
...
unsigned char closeSDOtransfer (CO_Data* d, unsigned char nodeId, unsigned char whoami)
{
  ...
  //Clear client SDO index 1280
  offset = d->firstIndex->SDO_CLT;
  if (offset == 0)return 0xFF;//No SDO client index found
...
unsigned char proceedSDO (CO_Data* d, Message *m)
{
  ...
  {
    MSG_WAR(0x3A69, "I am SERVER. Received SDO cobId : ", m->cob_id);// ERNAD: unsigned int_LE(m->cob_id));
  }
                                        ...
                                        unsigned char subIndex, unsigned char count, unsigned char dataType, void *data, SDOCallback_t Callback, unsigned char endianize)
{
  ...
  if (!err)
  {
    printf("SDO error : Communication yet established. with node : %u\n", nodeId);
    return 0xFF;
  }
  /* Taking the line ... */
  ...
  if (err)
  {
    printf("SDO error : No line free, too many SDO in progress. Aborted for node : %u\n", nodeId);
    return (0xFF);
  }
  ...
  if (offset == 0)
  {
	  printf("writeNetworkDict : No SDO client index found");
    return 0xFF;
  }
  i = 0;
  while (offset <= lastIndex) {
    if (d->objdict[offset].bSubCount <= 3)
    {
    	printf("Subindex 3  not found at index %u\n", 0x1280 + i);
      return 0xFF;
    }
  ...
  if (!SDOfound)
  {
    printf("SDO. Error. No client found to communicate with node : %u\n", nodeId);
    return 0xFE;
  }
...
#  endif
  ...
  { /** Normal transfer */
    ...
    for (i = 0 ; i < 4 ; i++)
//CHANGED 24112014 sdo.body.data[i+4] = (unsigned char)((count << (i<<3))); /* i*8 */
  ...
  if (err!=0x01)
  {
	  printf("SDO. Error while sending SDO to node : %u\n", nodeId);
    /* release the line */
    resetSDOline(d, line);
    return 0xFF;
  }
                                        ...
                                        unsigned long * abortCode)
{
  ...
  * abortCode = 0;
  ...
  if (err)
  {
    printf("SDO error : No line found for communication with node : %d\n", nodeId);
...
unsigned char getWriteResultNetworkDict (CO_Data* d, unsigned char nodeId, unsigned long * abortCode)
{
  ...
  * abortCode = 0;
  ...
  if (err)
  {
    printf("SDO error 2: No line found for communication with node : %d\n", nodeId);
    return SDO_ABORTED_INTERNAL;
  }
  * abortCode = d->transfers[line].abortCode;
  return d->transfers[line].state;
                                          ...
                                          unsigned char subIndex, unsigned long count, unsigned char dataType, void *data, SDOCallback_t Callback, unsigned char endianize)
{
  ...
  if(ret == 0xFE)
  {
    ...
    while (offset <= lastIndex)
    {
      if (d->objdict[offset].bSubCount <= 3)
      {
        MSG_ERR(0x1AC8, "Subindex 3  not found at index ", 0x1280 + nodeId);
        return 0xFF;
      }
