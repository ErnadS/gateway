...
void * stateWatchTask(void *ptr);
pthread_t threadCanStateWatch;
...
{
  unsigned int cob_id = UNS16_LE(m->cob_id);
...
unsigned char setState(CO_Data* d, e_nodeState newState)
{
  if(newState != d->nodeState)
  {
    ...
    case Stopped:
      if(d->nodeState == Initialisation) return 0xFF;
      {
    	  pthread_create(&threadCanStateWatch, NULL, stateWatchTask, (void*) d); // ernad
...
void * stateWatchTask(void *ptr) {
	CO_Data* d = (CO_Data*)ptr;
	int i;

	for (i = 0; i<30; i++) {
		sleep(1);
		if(d->nodeState != Stopped) {
			pthread_exit(0);
			return NULL;
		}
	}
...
{
  unsigned int offset = d->firstIndex->SDO_SVR;
  ...
  if(offset)
  {
    ...
    if((*(unsigned long*)d->objdict[offset].pSubindex[1].pObject == 0x600 + *d->bDeviceNodeId)||(*d->bDeviceNodeId==0xFF)){
      /* cob_id_client = 0x600 + nodeId; */
      *(unsigned long*)d->objdict[offset].pSubindex[1].pObject = 0x600 + nodeId;
    }
    /* Adjust COB-ID Server -> Client (tx) only id already set to default value or id not valid (id==0xFF)*/
    if((*(unsigned long*)d->objdict[offset].pSubindex[2].pObject == 0x580 + *d->bDeviceNodeId)||(*d->bDeviceNodeId==0xFF)){
      /* cob_id_server = 0x580 + nodeId; */
      *(unsigned long*)d->objdict[offset].pSubindex[2].pObject = 0x580 + nodeId;
    }
  }
  ...
  */
  {
    ...
    unsigned long cobID[] = {0x200, 0x300, 0x400, 0x500};
    if( offset ) while( (offset <= lastIndex) && (i < 4)) 
    {
      if((*(unsigned long*)d->objdict[offset].pSubindex[1].pObject == cobID[i] + *d->bDeviceNodeId)||(*d->bDeviceNodeId==0xFF))
        *(unsigned long*)d->objdict[offset].pSubindex[1].pObject = cobID[i] + nodeId;
      i ++;
      offset ++;
    }
  }
  ...
  {
    unsigned char i = 0;
    unsigned int offset = d->firstIndex->PDO_TRS;
    unsigned int lastIndex = d->lastIndex->PDO_TRS;
    unsigned long cobID[] = {0x180, 0x280, 0x380, 0x480};
    i = 0;
    if( offset ) while ((offset <= lastIndex) && (i < 4)) 
    {
      if((*(unsigned long*)d->objdict[offset].pSubindex[1].pObject == cobID[i] + *d->bDeviceNodeId)||(*d->bDeviceNodeId==0xFF))
