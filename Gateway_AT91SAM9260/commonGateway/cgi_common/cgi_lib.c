...
int parse_form_encoded(llist* entries)
{
  ...
  getline2(buffer,BUFSIZ);
  /* now start parsing */
  while ((bytesread=getline2(buffer,BUFSIZ)) != 0) {
    start = 1;
    ...
    *tempstr = '\0';
    ...
    while (getline2(buffer, BUFSIZ) > 1) {
    ...
    j = 0;
    while (!done) {
      bytesread = getline2(buffer,BUFSIZ);
      buffer[bytesread] = '\0';
...
int read_cgi_input(llist* entries)
{
  ...
  {
	  //printf("<h1>xx: parse_form_encoded</h1>\n");
  ...
  {
 	  //printf("<h1>xx: REQUEST_METHOD == NULL</h1>\n");
    input = get_DEBUG();
  }
  else if (!strcmp(REQUEST_METHOD,"POST"))
  {
	  //printf("<h1>xx: POST</h1>\n");
    input = get_POST();
  }
